...
DEVICE_SPECS = {
    ...
    'other': {
        'storage_mb': 128000,  # 128GB conservative estimate
        'memory_mb': 8192,     # 8GB conservative estimate
        'available_memory_ratio': 0.6  # 60% of memory can be used
    }
}
...
class SizeMetric(BaseMetric):
    ...
    def setup_resources(self):
        ...
        """
        try:
            self.device_specs = DEVICE_SPECS.get(self.target_platform, DEVICE_SPECS['other'])
    ...
    def calculate_score(self) -> float:
        ...
        if self.storage_size_mb > self.device_specs['storage_mb']:
            return 0.0
        ...
        available_memory = self.device_specs['memory_mb'] * self.device_specs['available_memory_ratio']
    ...
    def _fetch_model_info(self) -> dict:
        ...
        try:
            ...
            config_json_url = f"https://huggingface.co/{repo_id}/resolve/main/config.json"
    ...
    def _calculate_storage_size(self) -> float:
        ...
        bytes_per_param = TENSOR_TYPE_SIZES.get(tensor_type, TENSOR_TYPE_SIZES['default'])
    ...
    def _calculate_memory_size(self) -> float:
        ...
        activation_memory_bytes = seq_length * hidden_size * num_layers * bytes_per_activation
    ...
    def _estimate_parameters_from_config(self, cfg: dict) -> int:
        ...
        embedding_params = token_embeddings + position_embeddings + token_type_embeddings
    ...
    def get_size_details(self) -> dict:
        ...
        return {
            ...
            'device_storage_mb': self.device_specs['storage_mb'],
            'device_memory_mb': self.device_specs['memory_mb'],
            'available_memory_mb': self.device_specs['memory_mb'] * self.device_specs['available_memory_ratio'],
            'storage_fits': self.storage_size_mb <= self.device_specs['storage_mb'],
            'memory_fits': self.memory_size_mb <= (self.device_specs['memory_mb'] * self.device_specs['available_memory_ratio'])
        }
